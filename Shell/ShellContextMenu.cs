...
public class ShellContextMenu : NativeWindow
{
    ...
    private void InvokeContextMenuDefault(FileInfo[] arrFI)
    {
        ...
        try
        {
            ...
            if (false == GetContextMenuInterfaces(_oParentFolder, _arrPIDLs, out IntPtr iContextMenuPtr))
            {
                ReleaseAll();
                return;
            }
    ...
    public static bool Show(Point point, params FileInfo[] items)
    {
        try
        {
    ...
    public static bool Show(Point point, params DirectoryInfo[] items)
    {
        try
        {
            var instance = new ShellContextMenu();
    ...
    public static bool Show(Point point, params FileSystemInfo[] items)
    {
        try
        {
            var instance = new ShellContextMenu();
            ...
            foreach (var i in items)
            {
                ...
                if (i is DirectoryInfo folderInfo)
                {
                    folders.Add(folderInfo);
                }
            }

            var _files = instance.GetPIDLs(files.ToArray()) ?? [];
            var _folders = instance.GetPIDLs(folders.ToArray()) ?? [];
